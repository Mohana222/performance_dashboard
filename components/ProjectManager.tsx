        ...
        <div className="p-8 pt-2 overflow-y-auto flex-1 custom-scrollbar space-y-4">
              ...
              >
                {editingId === project.id ? (
                  <form onSubmit={(e) => handleUpdate(e, project)} className="space-y-4">
                    <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white outline-none focus:ring-2 focus:ring-violet-500" value={editName} onChange={e => setEditName(e.target.value)} required />
                    <input type="url" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-xs text-slate-400 font-mono outline-none focus:ring-2 focus:ring-violet-500" value={editUrl} onChange={e => setEditUrl(e.target.value)} required />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isAdmin && (
                        <>
                          <button onClick={() => startEditing(project)} className="p-2 text-slate-500 hover:text-white transition-colors" title="Edit">
                          ...
                          <button onClick={() => onDelete(project.id)} className="p-2 text-slate-500 hover:text-red-400 transition-colors" title="Delete">
          ...
          {isAdmin && showAdd && (
            <form onSubmit={handleSubmit} className="p-6 rounded-[1.5rem] bg-slate-900 border border-slate-700 space-y-4 animate-in slide-in-from-bottom-2">
              ...
              <input type="url" placeholder="Google Script URL" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-slate-400 font-mono outline-none focus:ring-2 focus:ring-violet-500" value={url} onChange={e => setUrl(e.target.value)} required />
              <div className="flex gap-2">
